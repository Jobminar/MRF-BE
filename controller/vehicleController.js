...
const vehicleController={
    ...
    createVehicle:async(req,res)=>{
        try{
         const {date,comment,vehicleName,vehicleModel,location,amount}=req.body
         if(!date || !comment || !vehicleName || !vehicleModel || !location || !amount){
            return res.status(201).json({message:"Required fields issing !!"})
         }
         const newVehicle=new Vehicle({date,comment,vehicleName,vehicleModel,location,amount})
